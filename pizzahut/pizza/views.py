...
def order(request):
    ...
    if request.method == 'POST':
        filled_form = PizzaForm(request.POST)  #filled form instance
        if filled_form.is_valid():
            note = 'Thanks your order for %s, %s and %s size pizza was placed'%(filled_form.cleaned_data['topping1'],
...
def edit(request,pk):
    ...
    form = PizzaForm(instance=pizza)      #placed order from was collect
    if request.method == 'POST':
